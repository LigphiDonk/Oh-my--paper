...
use crate::services::openclaw_lifecycle;

#[tauri::command]
pub fn get_openclaw_status(
    state: State<'_, openclaw_lifecycle::OpenClawGatewayState>,
) -> Result<openclaw_lifecycle::GatewayStatus, String> {
    Ok(openclaw_lifecycle::get_status(&state))
}

#[tauri::command]
pub fn start_openclaw_gateway(
    app_handle: AppHandle,
    oc_state: State<'_, openclaw_lifecycle::OpenClawGatewayState>,
) -> Result<openclaw_lifecycle::GatewayStatus, String> {
    let app_state = app_handle.state::<AppState>();
    let root_path = app_state
        .project_config
        .read()
        .map_err(|e| e.to_string())?
        .root_path
        .clone();
    let workspace = if root_path.trim().is_empty() {
        None
    } else {
        Some(std::path::PathBuf::from(&root_path))
    };
    openclaw_lifecycle::start_gateway(&oc_state, workspace.as_deref())?;
    Ok(openclaw_lifecycle::get_status(&oc_state))
}

#[tauri::command]
pub fn restart_openclaw_gateway(
    app_handle: AppHandle,
    oc_state: State<'_, openclaw_lifecycle::OpenClawGatewayState>,
) -> Result<openclaw_lifecycle::GatewayStatus, String> {
    let app_state = app_handle.state::<AppState>();
    let root_path = app_state
        ...
        .clone();
    if root_path.trim().is_empty() {
        return Err("No active project to use as workspace".into());
    }
    openclaw_lifecycle::restart_with_workspace(&oc_state, std::path::Path::new(&root_path))?;
    Ok(openclaw_lifecycle::get_status(&oc_state))
}
