...
|-------|---------|---------------|
| survey | 文献搜索 | `/omp:survey` 或直接 dispatch `literature-scout` persona |
| ideation | 创新点生成+评估 | `/omp:ideate` |
| experiment | 实验设计/实现/运行/分析 | `/omp:experiment`，循环直到达标 |
| publication | 论文写作 + 质量审查 | `/omp:write` → `/omp:review` |
